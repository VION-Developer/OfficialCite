// Round Carousel — Originkit (Clics Corregidos)
"use client";
...
export default function RoundCarousel({
  ...
  // Ref para detectar si el usuario arrastró o hizo clic
  ...
  const onPointerMove = (e: React.PointerEvent) => {
    ...
    // Si el usuario movió el mouse más de 3px, se considera arrastre (giro)
    if (Math.abs(dx) > 3) {
      d.moved = true;
    }
  ...
  const handleCardClick = (itemOnClick?: () => void) => {
    // Si NO movió el mouse (fue un clic puro), ejecuta el evento
    if (!dragRef.current.moved && itemOnClick) {
      itemOnClick();
    }
  };
        ...
        >
          {items.map((img, i) => {
            ...
            return (
              <div
                key={i}
                onClick={() => handleCardClick(img.onClick)}
                style={{
                  position: "absolute",
                  inset: 0,
